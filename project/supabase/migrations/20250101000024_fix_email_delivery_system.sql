...
-- 3. Convert enum columns back to TEXT if they exist as enums
-- This ensures we can work with simple text values
DO $$
DECLARE
    col_type text;
BEGIN
...
DECLARE
    col_type text;
BEGIN
    -- Check email_status column type
    SELECT data_type INTO col_type
    FROM information_schema.columns 
    WHERE table_name = 'admin_email_notifications' 
    ...
    IF col_type != 'text' THEN
        ALTER TABLE admin_email_notifications 
        ALTER COLUMN email_status TYPE TEXT 
        USING email_status::text;
    END IF;
END $$;

-- 4. Update existing records to have proper email_type and email_status
...
-- 5. Set proper priorities for different email types
UPDATE admin_email_notifications 
SET priority = CASE 
...
-- 6. Create indexes for better email processing performance
...
-- 7. Create RLS policies for email configuration table
...
-- 8. Insert default email configuration (Gmail SMTP for testing)
...
-- 9. Create function to process email queue
...
-- 11. Create view for email delivery monitoring
