/*
  # Add Coordinate Fields to Bookings Tables

  This migration adds coordinate and postcode fields to both bookings and guest_bookings tables
...
-- Add coordinate fields to bookings table
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE bookings ADD COLUMN pickup_lat DECIMAL(10,8);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE bookings ADD COLUMN pickup_lng DECIMAL(11,8);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'dropoff_lat'
  ) THEN
    ALTER TABLE bookings ADD COLUMN dropoff_lat DECIMAL(10,8);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'dropoff_lng'
  ) THEN
    ALTER TABLE bookings ADD COLUMN dropoff_lng DECIMAL(11,8);
  END IF;
  
  -- Add postcode fields if they don't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'pickup_postcode'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'dropoff_postcode'
  ) THEN
    ALTER TABLE bookings ADD COLUMN dropoff_postcode TEXT;
  END IF;
  
  -- Add Google Place ID fields if they don't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'pickup_place_id'
  ) THEN
    ALTER TABLE bookings ADD COLUMN pickup_place_id TEXT;
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'bookings' AND column_name = 'dropoff_place_id'
  ) THEN
    ALTER TABLE bookings ADD COLUMN dropoff_place_id TEXT;
  END IF;
END $$;
...
COMMENT ON COLUMN bookings.pickup_lat IS 'Latitude of pickup location from Google Places';
COMMENT ON COLUMN bookings.pickup_lng IS 'Longitude of pickup location from Google Places';
COMMENT ON COLUMN bookings.dropoff_lat IS 'Latitude of dropoff location from Google Places';
COMMENT ON COLUMN bookings.dropoff_lng IS 'Longitude of dropoff location from Google Places';
COMMENT ON COLUMN bookings.pickup_postcode IS 'Postcode extracted from pickup address';
COMMENT ON COLUMN bookings.dropoff_postcode IS 'Postcode extracted from dropoff address';
COMMENT ON COLUMN bookings.pickup_place_id IS 'Google Place ID for pickup location';
COMMENT ON COLUMN bookings.dropoff_place_id IS 'Google Place ID for dropoff location';
