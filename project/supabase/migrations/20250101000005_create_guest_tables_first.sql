/*
  # Create Guest Tables First - Foundation Migration
  
  This migration creates the essential guest booking tables before any other
  migrations try to modify them. This ensures the database schema is properly
  established.
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.guest_riders ADD CONSTRAINT guest_riders_email_key UNIQUE (email);
  END IF;
END $$;
...
DO $$
BEGIN
  RAISE LOG 'Guest tables foundation migration completed successfully';
  RAISE LOG 'All required tables have been created with proper structure';
